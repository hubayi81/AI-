...
> ⚠️ **本文档是 2026-05 立项阶段的初始设计快照，不是当前实现的准确描述。**
> **实现以 [README.md](./README.md) 为准。** 保留它是为了展示"先定义需求、再实现"的过程。
>
> 主要差异（设计 → 落地过程中改过的）：
>
> | 项 | 本文档（5 月设计） | 当前实现 |
> |---|---|---|
> | Python 框架 | Flask | **FastAPI** |
> | 工具数量 | 4 个 | **5 个**（新增 `search_knowledge`） |
> | 对话记忆 | 内存存储，重启丢失 | **Redis（TTL 24h）+ MySQL 持久化** |
> | 商品获取 | Java 每请求全量传 353 款 | **Python 直连 MySQL（架构 A）** |
> | 推荐评分 | 未定义 | **系统计算 `scoring.score_product`，非 LLM 生成** |
> | 服务入口 | `python app.py` | `python main.py` |
...
一个 **AI Agent 对话推荐系统**：用户用自然语言和 AI 聊天，AI 自主决策调用工具（搜索商品、分析穿搭、追问偏好、对比商品），最终从真实数据库中找到匹配的鞋款并给出个性化推荐理由。
